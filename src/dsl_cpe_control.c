...
#ifdef CONFIG_OPENRG
static int lantiq_vr9_adsl_status_report(char *st)
{
    int status, rc, fd, i;
    ...
    for (i = 0; status2code[i].str; i++)
    {
	if (!strcmp(status2code[i].str, st))
	    break;
    }

    if ((status = status2code[i].code) == -1)
	return -1;

    if (last_status == status)
        return 0;

    if ((fd = ipc_connect(htons(RG_IPC_PORT_LANTIQ_DSL))) < 0)
	return -1;

    rc = ipc_write(fd, &status, sizeof(status));
    last_status = status;
...
static int lantiq_dsl_update_sync_status(char *st)
{
    FILE *f = fopen("/tmp/dsl_status", "w");
    ...
    return lantiq_vr9_adsl_status_report(st);
}
#else
#define lantiq_dsl_update_sync_status(st)
#endif
   ...
   DSL_char_t *pcFw2)
{
   ...
   */
   ...
   if (nRet >= DSL_SUCCESS)
       lantiq_dsl_update_sync_status("autoboot_done");
   ...
   DSL_EventStatus_t *pEvent)
{
...
#endif
   {
           ...
           (g_nPrevLineState[nDevice] != DSL_LINESTATE_SILENT) )
      {
         ...
         {
            bExec = DSL_TRUE;
	    lantiq_dsl_update_sync_status("ready");
         }
      }
                ...
                (g_nPrevLineState[nDevice] != DSL_LINESTATE_HANDSHAKE) )
      {
         ...
         {
            bExec = DSL_TRUE;
	    lantiq_dsl_update_sync_status("handshake");
         }
      }
                ...
                (g_nPrevLineState[nDevice] != DSL_LINESTATE_FULL_INIT) )
      {
         ...
         {
            bExec = DSL_TRUE;
	    lantiq_dsl_update_sync_status("training");
         }
      }
           ...
           (g_nPrevLineState[nDevice] != DSL_LINESTATE_SHOWTIME_TC_SYNC) )
      {
         ...
         {
            bExec = DSL_TRUE;
	    lantiq_dsl_update_sync_status("showtime");
         }
      }
                 ...
                 (g_nPrevLineState[nDevice] == DSL_LINESTATE_SHOWTIME_TC_SYNC)) )
      {
         ...
         {
            bExec = DSL_TRUE;
	    lantiq_dsl_update_sync_status("down");
         }
      }
#if (DSL_CPE_MAX_DSL_ENTITIES > 1)
